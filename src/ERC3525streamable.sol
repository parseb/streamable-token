...
import "./needed/IERC3525.sol";
import "./needed/StringConvertor.sol";
import "./needed/IERC3525Receiver.sol";
...
contract ERC3525streamable is IERC3525, Context {
    ...
    function approve(uint256 tokenId_, address to_, uint256 value_) public payable virtual override {
        address owner = ERC3525streamable.ownerOf(tokenId_);
        require(to_ != owner, "ERC3525: approval to current owner");

        require(
            _msgSender() == owner || ERC3525streamable.isApprovedForAll(owner, _msgSender()),
        ...
        returns (uint256)
    {
        ...
        _mintValue(to_, ERC3525streamable.slotOf(fromTokenId_), newTokenId, 0);
    ...
    function tokenByIndex(uint256 index_) public view virtual returns (uint256) {
        require(index_ < ERC3525streamable.totalSupply(), "ERC3525: global index out of bounds");
    ...
    function tokenOfOwnerByIndex(address owner_, uint256 index_) public view virtual returns (uint256) {
        require(index_ < ERC3525streamable.balanceOf(owner_), "ERC3525: owner index out of bounds");
    ...
    function _isApprovedOrOwner(address operator_, uint256 tokenId_) internal view virtual returns (bool) {
        ...
        address owner = ERC3525streamable.ownerOf(tokenId_);
        return (
            operator_ == owner || ERC3525streamable.isApprovedForAll(owner, operator_)
    ...
    function _spendAllowance(address operator_, uint256 tokenId_, uint256 value_) internal virtual {
        uint256 currentAllowance = ERC3525streamable.allowance(tokenId_, operator_);
    ...
    function _mintValue(address to_, uint256 slot_, uint256 tokenId_, uint256 value_)
        internal
        virtual
        returns (uint256)
    {
    ...
    function _approve(address to_, uint256 tokenId_) internal virtual {
        ...
        emit Approval(ERC3525streamable.ownerOf(tokenId_), to_, tokenId_);
    }
    ...
    function _transferTokenId(address from_, address to_, uint256 tokenId_) internal virtual {
        require(ERC3525streamable.ownerOf(tokenId_) == from_, "ERC3525: transfer from incorrect owner");
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual returns (bool) {
        return interfaceId == type(IERC3525).interfaceId || interfaceId == type(IERC721).interfaceId;
    }
}
